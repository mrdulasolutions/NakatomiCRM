...
Nakatomi is a **headless, agent-native CRM** (v1.0.6). REST + MCP + A2A + ACP on Postgres — built for Claude, ChatGPT, Cursor, and custom agent swarms.
...
After deploy, open `/health` — expect `"version":"1.0.6"`. First visit to `/` shows the branded welcome flow (workspace + API key).
